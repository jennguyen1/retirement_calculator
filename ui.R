    ...
    box(
      ...
      tabBox(width = NULL, selected = "Input Data",
        ...
        tabPanel("Instructions",
          ...
          p("The program uses the following algorithm for determining if a Roth Ladder Conversion is needed and when it should be applied. If the taxable accounts total is less than 6 times the spending per year and there is more than 5 years before one can access the retirement accounts without penalty, then the program recommends starting the Roth Ladder on that year so that the retirement accounts can be accessed 5 years later. The program will issue a warning if the taxable account balances are too low to sustain the spending per year for 5 years necessary to adequately setup the Roth Ladder. Once the retirement accounts can be accessed, the taxable account balances are transferred to the retirement accounts and all further withdrawals occur in the retirement accounts. The program will issue a warning if the retirement accounts are depleted before age 100."),
